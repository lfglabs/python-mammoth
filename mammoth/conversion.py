...
class _DocumentConverter(documents.element_visitor(args=1)):
    ...
    def get_css_border_value_from_xml_attributes(self, attributes):
        # Potential values for w:val are 'none', 'single', 'double', etc.
        # Only support 'none' and 'single'. Default others to 'single'
        if attributes.get('w:val') == 'none' or \
            attributes.get('w:color') == None or \
            attributes.get('w:sz') == None:
            return 'none'
    ...
    def get_cell_border_style_from_border_properties(self, attributes_map):
        ...
        for key in documents.TableCell.BORDER_XML_TO_CSS_PROPERTY_MAP:
            if attributes_map.get(key):
                ...
                key = documents.TableCell.BORDER_XML_TO_CSS_PROPERTY_MAP[key]
                value = self.get_css_border_value_from_xml_attributes(attributes)
